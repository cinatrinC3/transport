...
故統合為一個如$h$一般的質傳係數$k_c,k_y,k_p$\\
這些數值通常無法直接計算，只能透過實驗或邊界條件下的質量或能量守恆來得到:
\begin{equation}
  \boxed{N_A=k_c\Delta(C_A) = k_y \Delta (y_A) = k_p \Delta (P_A)}
\end{equation}
$k_c,k_y,k_p$分別為以濃度、莫耳分率、分壓表示的質傳係數\\
而對比熱對流會用Nusselt Number或Stanton Number (Heat) 來求出$h$\\
